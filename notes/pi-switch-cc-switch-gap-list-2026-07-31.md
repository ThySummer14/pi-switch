...
| P1 | 操作反馈 | 已覆盖连接测试、模型同步和 Doctor 的阶段状态；后续只需随新操作继续复用阶段组件 | 各操作显示可区分的进行中/成功/失败状态和可读错误 |
...
| P2 | 列表体验 | Provider/模型搜索已存在；模型列表超过 120 条时已启用无依赖窗口化渲染，MCP/Provider 仍是普通列表 | 长列表滚动稳定，空/错/加载状态一致 |
...
- Doctor 现在把本地配置和 Provider 网络检查作为独立阶段返回耗时、错误/警告摘要和跳过原因；桌面弹窗可以区分本地问题与网络问题，CLI `runDoctor()` 的原数组返回保持不变。
- 模型同步现在展示模型列表读取、能力声明分析和写入配置三个阶段；模型列表失败不会再被误认为“没有新模型”，写入失败会保留失败阶段和可读错误，用户可以在弹窗中查看后关闭。
- 2026-08-01 验证：`npm test` 128/128；新增有效 Provider 网络检查、模型同步失败阶段和 Doctor bridge 阶段回归。

## 2026-08-01 超长模型列表

- Models 页面在筛选结果超过 120 条时启用窗口化渲染，只挂载可视区域和少量 overscan；搜索、筛选、选择、批量编辑、激活、编辑和删除仍使用同一行操作。
- 窗口化列表使用固定行高、独立滚动容器和 `ResizeObserver`，移动端按卡片行高切换；筛选或 Provider 变化会复位滚动位置。未增加第三方依赖，短列表保持原有 DOM 和布局。
- 2026-08-01 验证：`npm run build:web`、`npm test`（129/129）和 `git diff --check` 通过；桌面包签名复核仍为 arm64 ad-hoc。
